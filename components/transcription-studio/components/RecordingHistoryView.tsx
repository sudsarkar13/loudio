...
interface RecordingHistoryViewProps {
	...
	onMigrateLegacyRecordings: () => Promise<void>;
}
		...
		<>
					...
					<button
						...
						onClick={() =>
							void loadRecordingHistory({
								statusMessage: "Recording history refreshed.",
							})
						}
						disabled={
							isLoadingRecordingHistory || deletingRecordingPath !== null
						}>
						Refresh
					</button>
				</div>
			...
			<div className="history-bulk-bar">
				...
				<span className="history-selection-count">
					{hasSelectedRecordings ?
						`${selectedRecordingPaths.length} selected`
					:	"No selection"}
				</span>
			</div>

			{legacyDirs.length > 0 ?
				<div className="history-legacy-banner" role="status" aria-live="polite">
					<div className="history-legacy-banner-text">
						<ArrowRightLeft size={16} />
						<div>
							<p className="history-legacy-banner-title">
								Found {legacyDirs.length} older Loudio{" "}
								{legacyDirs.length === 1 ? "folder" : "folders"} with{" "}
								{formatRecordingSize(legacyDiskUsageBytes)} of microphone
								recordings.
							</p>
							<p className="history-legacy-banner-detail">
								These files are stranded in old app data folders and won't show
								up in this list. Move them into the current Loudio folder to
								bring them back and reclaim the space.
							</p>
						</div>
					</div>
					<button
						className="btn btn-primary compact-toggle-btn"
						onClick={() => void onMigrateLegacyRecordings()}
						disabled={isMigratingLegacyRecordings}>
						{isMigratingLegacyRecordings ?
							"Moving…"
						:	`Move ${formatRecordingSize(legacyDiskUsageBytes)} into Loudio`}
					</button>
				</div>
			:	null}

			<div className="history-storage-row">
				<HardDrive size={14} />
				<span>
					Using {formatRecordingSize(recordingsDiskUsageBytes)} in this app
					{legacyDiskUsageBytes > 0 ?
						<span className="history-storage-legacy">
							{" "}
							+ {formatRecordingSize(legacyDiskUsageBytes)} stranded in older
							folders
						</span>
					:	null}
				</span>
			...
			{activePlaybackItem ?
				<div className="history-player" aria-live="polite">
					<div className="history-player-main">
						<p
							className="history-player-title"
							title={activePlaybackItem.fileName}>
						...
						<div className="history-player-controls">
							...
							<button
								...
								disabled={
									isDeletingSelectedRecordings || deletingRecordingPath !== null
								}>
								{isPlaybackPlaying ?
									<>
										<Pause size={14} /> Pause
					...
					<div className="history-player-timeline">
						<span className="history-time">
							{formatPlaybackTime(playbackCurrentSec)}
						</span>
						<input
							type="range"
							min={0}
			...
			:	<div
					...
					role="list"
					aria-label="Microphone recording history">
					{recordingHistory.map((item: RecordingHistoryItem) => {
						const deleting: boolean =
							deletingRecordingPath === item.absolutePath;
						const selected: boolean = selectedRecordingPaths.includes(
							item.absolutePath,
						);
							...
							<article className="history-item" key={item.id} role="listitem">
								...
								<div className="history-item-main">
									...
									<p className="history-meta">
										{formatRecordingSize(item.sizeBytes)} ·{" "}
										{item.extension.toUpperCase()} ·{" "}
										{formatRecordingDate(item.createdAtIso)}
									</p>
								</div>
								...
								<div className="history-item-actions">
									<button
										...
										title={
											playing && isPlaybackPlaying ? "Pause playback" : (
												"Play recording"
											)
										}
										aria-label={
