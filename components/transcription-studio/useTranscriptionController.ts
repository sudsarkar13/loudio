...
import type { AppSettings, TranscriptionResponse } from "@/lib/types";
import {
	appendTranscriptText,
	normalizeTranscriptText,
} from "@/components/transcription-studio/utils/transcript";
...
export function useTranscriptionController(): UseTranscriptionControllerReturn {
	...
	const [status, setStatus] = useState<string>(
		"Choose an audio file or start the microphone to begin.",
	);
	...
	async function onTranscribe(
		...
		try {
			const response: TranscriptionResponse = await startTranscription(
				audioPath,
				settings,
			);
			const normalizedText: string = normalizeTranscriptText(response.text);
			const normalizedResponse: TranscriptionResponse = {
				...response,
				text: normalizedText,
			};
	...
	async function transcribeMicrophoneBlob(
		blob: Blob,
		settings: AppSettings,
	): Promise<void> {
		setResult(null);
		...
		try {
			const response: TranscriptionResponse =
				await startMicrophoneTranscription(blob, settings);
			const normalizedText: string = normalizeTranscriptText(response.text);
			const normalizedResponse: TranscriptionResponse = {
				...response,
				text: normalizedText,
